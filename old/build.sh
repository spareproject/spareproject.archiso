...
make_basefs() { mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" init; }
make_packages() { mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages)" install; }
#########################################################################################################################################################################################
make_setup_mkinitcpio() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux -g /boot/archiso.img' run
    #mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-grsec -g /boot/archiso-grsec.img' run
    #mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts -g /boot/archiso.img' run
}
#########################################################################################################################################################################################
make_customize_airootfs() {
    ...
    mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${pacman_conf}" -D "${install_dir}" -r '/root/customize_airootfs.sh' run
...
make_boot() {
    ...
    #cp ${work_dir}/${arch}/airootfs/boot/archiso-grsec.img ${work_dir}/iso/${install_dir}/boot/${arch}/archiso-grsec.img
    #cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-grsec ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz-grsec
    #cp ${work_dir}/${arch}/airootfs/boot/vmlinuz-linux-lts ${work_dir}/iso/${install_dir}/boot/${arch}/vmlinuz
}
#########################################################################################################################################################################################
make_boot_extra() {
    # does linux-firmware need a copy here as well?
...
make_syslinux() {
    ...
    cp ${work_dir}/${arch}/airootfs/usr/lib/syslinux/bios/lpxelinux.0 ${work_dir}/iso/${install_dir}/boot/syslinux # pxe?
    cp ${work_dir}/${arch}/airootfs/usr/lib/syslinux/bios/memdisk ${work_dir}/iso/${install_dir}/boot/syslinux # memdisk?
    # left over from hardware test uninstalled the packages so no point having same with pxe leftovers...
...
make_efiboot() {
    ...
    #cp ${work_dir}/iso/${install_dir}/boot/x86_64/vmlinuz-grsec ${work_dir}/efiboot/EFI/archiso/vmlinuz-grsec.efi
    #cp ${work_dir}/iso/${install_dir}/boot/x86_64/archiso-grsec.img ${work_dir}/efiboot/EFI/archiso/archiso-grsec.img

    #apperently this cant be found
...
make_iso() { mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "${iso_name}-${iso_version}-dual.iso"; }
#########################################################################################################################################################################################
...
#########################################################################################################################################################################################
if [[ -d ./gnupg/admin && -d ./gnupg/spareproject ]];then
  rm -r ./work/x86_64/airootfs/home/admin/.gnupg
  cp -ar ./gnupg/admin ./work/x86_64/airootfs/home/admin/.gnupg
  chroot ./work/x86_64/airootfs chown -R admin:wheel /home/admin/.gnupg
  chroot ./work/x86_64/airootfs chmod -R 700 /home/admin/.gnupg
  rm -r ./work/x86_64/airootfs/home/spareproject/.gnupg
  cp -ar ./gnupg/spareproject ./work/x86_64/airootfs/home/spareproject/.gnupg
  chroot ./work/x86_64/airootfs chown -R spareproject:users /home/spareproject/.gnupg
  chroot ./work/x86_64/airootfs chmod -R 700 /home/spareproject/.gnupg
fi 
#########################################################################################################################################################################################
ask() {
    while true; do
...
if ask "generate iso image or play with airootfs..."; then
  rm ./work/x86_64/airootfs/etc/machine-id
  run_once make_prepare
  run_once make_iso
else
  echo "skipping iso gen"
fi
#########################################################################################################################################################################################
